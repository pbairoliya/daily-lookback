...
def test_payee_core_drops_card_digits():
    assert bill_payee_core("Pay credit card — Bank of America …2222 ($500, due Jun 12)") == "bank of america"
...
def test_paid_bill_suppressed_same_cycle_only(fake_env):
    ...
    assert is_paid_bill("Pay credit card — Bank of America …2222 ($500, due June 12, 2026)", TODAY, keys)
...
def test_strip_removes_paid_bill_already_in_note(fake_env):
    ...
    note = _tasks_note(open_=[
        "Pay credit card — Bank of America …2222 ($500, due June 12, 2026)",  # paid
